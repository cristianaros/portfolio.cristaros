...
## 2024-05-15 - Accesibilidad y teclado en controles de juegos embebidos
**Learning:** Los controles direccionales para juegos embebidos (como el D-pad de Snake) a menudo usan `onMouseDown` para una respuesta rápida, pero carecen de `onClick`, lo que rompe la navegación por teclado (Enter/Espacio).
**Action:** Al implementar controles interactivos nativos (`<button>`), siempre incluir manejadores `onClick` como respaldo de `onMouseDown`, junto con atributos descriptivos (`aria-label`, `title` en español) y estilos visibles de foco.

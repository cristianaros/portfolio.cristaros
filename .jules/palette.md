...
## 2024-05-15 - Accesibilidad por teclado en controles con onMouseDown
**Learning:** Los controles interactivos (como el D-pad de Snake) que utilizan `onMouseDown` para interacciones rápidas con el ratón no pueden ser activados mediante teclado (Enter/Espacio) si carecen del evento `onClick`, lo que rompe la accesibilidad por teclado.
**Action:** Siempre incluir un manejador `onClick` junto a los manejadores `onMouseDown` en elementos `<button>` nativos para garantizar que la acción pueda ser disparada de manera accesible al recibir foco.

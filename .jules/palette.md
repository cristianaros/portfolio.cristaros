...
## 2024-05-11 - Accesibilidad de teclado en controles de juegos embebidos
**Learning:** Los controles interactivos de juegos (como D-Pads) que dependen exclusivamente del evento `onMouseDown` para reducir latencia son inaccesibles mediante el teclado (teclas Enter/Espacio), excluyendo a usuarios que no utilizan ratón.
**Action:** Siempre incluir un manejador `onClick` complementario junto a `onMouseDown` en elementos `<button>` nativos para garantizar que la acción pueda ser disparada correctamente cuando el elemento tiene el foco del teclado.

...
const Line: React.FC<LineProps> = ({ number, children, className = "" }) => (
  <div
    className={`flex items-start hover:bg-white/[0.02] transition-colors ${className}`}
  >
...
export default function Hero() {
  ...
  const Wrap: React.FC<{ delay: number; children: React.ReactNode }> = ({
    delay,
    children,
  }) => {
      ...
      <Line number={9}>
        <Wrap delay={700}>
          <p className="text-base md:text-lg text-vscode-textSubtle leading-relaxed max-w-2xl">
            ...
            enfocado en construir aplicaciones web modernas, escalables y con
            arquitectura limpia. Busco crecer como{" "}
            <span className="text-vscode-green font-semibold">
              desarrollador full stack
            </span>
            , priorizando código limpio, buenas prácticas y diseño centrado en
            el usuario.
          </p>
        </Wrap>
      </Line>

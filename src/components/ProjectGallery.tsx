...
function hasAnimated(): boolean {
  try {
    return sessionStorage.getItem("projects-animated") === "true";
  } catch {
    return false;
  }
}
    ...
    <div className="p-5 flex flex-col gap-3">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          <svg
            className="w-5 h-5 text-vscode-accent shrink-0"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <span
          className={`badge ${categoryColors[project.category] || "badge-blue"} text-[10px]`}
        >
          {project.category}
        </span>
      </div>

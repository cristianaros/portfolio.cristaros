...
function drawGame(canvas: HTMLCanvasElement, snake: Point[], food: Point) {
...
export default function SnakeInline() {
  ...
  const doGameOver = () => {
    ...
    if (
      g.score > parseInt(localStorage.getItem("snake-high-score") || "0", 10)
    ) {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case "w":
          e.preventDefault();
          if (dir !== "DOWN") g.nextDirection = "UP";
          break;
        case "ArrowDown":
        case "s":
          e.preventDefault();
          if (dir !== "UP") g.nextDirection = "DOWN";
          break;
        case "ArrowLeft":
        case "a":
          e.preventDefault();
          if (dir !== "RIGHT") g.nextDirection = "LEFT";
          break;
        case "ArrowRight":
        case "d":
          e.preventDefault();
          if (dir !== "LEFT") g.nextDirection = "RIGHT";
          break;
      }
    };
  ...
  return (
    <div className="flex items-center justify-center h-full w-full overflow-auto py-6 px-4">
      {/* Nokia Phone Frame */}
      <div
        className="bg-[#2c2c3a] rounded-[2rem] p-4 sm:p-6 shadow-2xl border border-[#45475a] max-w-full"
        style={{ minWidth: "min(340px, 100%)" }}
      >
        {/* Brand */}
        <div className="text-center mb-3">
          <span className="text-[13px] tracking-[0.3em] uppercase text-[#6c7086] font-bold">
            PORTFOLIO
          </span>
        </div>
        ...
        <div className="flex justify-center gap-8 mt-3">
          ...
          <button className="w-10 h-10 rounded-full bg-[#45475a] hover:bg-[#585b70] active:bg-[#313244] flex items-center justify-center text-[10px] text-[#cdd6f4] font-bold transition-colors">
            B
          </button>
        </div>
